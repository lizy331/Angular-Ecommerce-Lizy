...
export class CheckoutComponent implements OnInit {
  ...
  onSubmit(){
    ...
    for(let i=0; i<cartItems.length;i++ ){
        ...
        // console.log("add an order Item: " + orderItems[i].unitPrice)
    }
    ...
    console.log("review the purchase info: " + JSON.stringify(purchase,null,3))


    // call the REST API via checkout service
    this.checkoutService.placeHolder(purchase).subscribe({
      next: response => {
